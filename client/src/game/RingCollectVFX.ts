...
export class RingCollectVFX {
  ...
  private shardGeo: BufferGeometry;
  ...
  private createInstance(): VFXInstance {
    ...
    const shards = new InstancedMesh(this.shardGeo, shardMat, SHARD_COUNT);
  ...
  play(worldPos: Vector3, _tier: number) {
    ...
    for (let i = 0; i < SHARD_COUNT; i++) {
      ...
      _dummy.scale.setScalar(1.0 + Math.random() * 0.8);
  ...
  update(dt: number) {
    for (const inst of this.pool) {
      ...
      inst.shardMat.uniforms.globalAlpha.value = fadeSmooth;

      for (let i = 0; i < SHARD_COUNT; i++) {
        const s = inst.states[i];
        const t = inst.life;

        const gravOffset = 0.5 * GRAVITY * t * t;

        const px = inst.center.x + s.velocity.x * t + inst.upDir.x * gravOffset;
        const py = inst.center.y + s.velocity.y * t + inst.upDir.y * gravOffset;
        const pz = inst.center.z + s.velocity.z * t + inst.upDir.z * gravOffset;

        s.rotation.x += s.rotVelocity.x * dt;
        s.rotation.y += s.rotVelocity.y * dt;
        s.rotation.z += s.rotVelocity.z * dt;

        const scale = fadeSmooth * (0.8 + Math.random() * 0.05);
